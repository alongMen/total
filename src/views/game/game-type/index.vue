...
export default {
  ...
  methods: {
    ...
    async edit(row) {
     ...
     async setState(value,row,index){
      ...
      this.$Modal.confirm({
        title: `${title}`,
        loading: true,
        content: `<p>是否${title}该游戏类型</p>`,
        onOk: async () => {
          this.btnLoading = true;
          const res = await set_state_game_type(row.id, state);
          this.btnLoading = false;
          if (res.status < 400) {
            this.$Message.success(`${title}游戏类型成功`);
            this.$Modal.remove();
            // this.getData();
          } else {
            if (value === 1) {
    ...
    async ok() {
      ...
      this.btnLoading = true;
      if (this.form.id) {
        const res = await update_game_type(this.form.id,this.form);
        if (res.status < 400) {
          this.$Message.success('修改游戏类型成功');
        }
      } else {
        const res = await create_game_type(this.form);
        if (res.status < 400) {
          this.$Message.success('添加游戏类型成功');
        }
      }
      this.btnLoading = false;
    ...
    async getData() {
      this.loading = true;
      const res = await get_game_type_page(this.page, this.limit, this.query);
      this.loading = false;
      if (res.status < 400) {
